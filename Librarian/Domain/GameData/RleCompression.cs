...
namespace Nyerguds.GameData.Compression
{
    ...
    {
        protected override bool GetCode(byte[] buffer, ref uint inPtr, uint bufferEnd, out bool isRepeat, out uint amount)
        {
            bool success = base.GetCode(buffer, ref inPtr, bufferEnd, out isRepeat, out amount);
            isRepeat = !isRepeat;
            return success;
        }
    ...
    public abstract class RleImplementation<T> where T : RleImplementation<T>, new()
    {
        ...
        public virtual uint MaxRepeatValue { get { return 0x7F; } }
        /// <summary>Maximum amount of copied bytes that can be stored in one code.</summary>
        public virtual uint MaxCopyValue { get { return 0x7F; } }
        ...
        protected virtual bool GetCode(byte[] buffer, ref uint inPtr, uint bufferEnd, out bool isRepeat, out uint amount)
        {
            if (inPtr >= bufferEnd)
            {
                isRepeat = false;
        ...
        public byte[] RleDecodeData(byte[] buffer, uint? startOffset, uint? endOffset, int decompressedSize, bool abortOnError)
        {
            ...
            int result = RleDecodeData(buffer, startOffset, endOffset, ref outputBuffer, abortOnError);
            if (result == -1)
                return null;
            return outputBuffer;
        }
        ...
        public int RleDecodeData(byte[] buffer, uint? startOffset, uint? endOffset, ref byte[] bufferOut, bool abortOnError)
        {
            ...
            while (inPtr < inPtrEnd && outPtr < maxOutLen)
            {
                ...
                uint run;
                bool repeat;
                if (!GetCode(buffer, ref inPtr, inPtrEnd, out repeat, out run) || (run == 0 && abortOnError))
                {
                    error = true;
                    break;
                }
        ...
        public byte[] RleEncodeData(byte[] buffer)
        {
            ...
            uint bufLen = (uint)((buffer.Length * 3) / 2);
            ...
            uint maxRepeat = MaxRepeatValue;
            uint maxCopy = MaxCopyValue;
        ...
        private byte[] ExpandBuffer(byte[] bufferOut, uint expandSize)
        {
